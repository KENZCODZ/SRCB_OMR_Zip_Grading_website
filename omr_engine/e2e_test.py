...
Tests:
  ...
  7. Delete exam (DELETE /api/exams/{id})
"""
...
with open(IMAGE_PATH, "rb") as f:
    r = client.post("/api/extract", files={"file": ("sheet.png", f.read(), "image/png")})
test("Status code is 200", r.status_code == 200, f"got {r.status_code}")
extract_data = r.json()
...
for q, v in extract_data.get("answers", {}).items():
    if v and isinstance(v, str) and len(v) == 1 and v.upper() in ["A", "B", "C", "D", "E"]:
        answer_key[q] = v.upper()

# Fallback answer key if sheet didn't have valid bubbles
if not answer_key:
...
with open(IMAGE_PATH, "rb") as f:
    r = client.post(
        ...
        files={"file": ("student_sheet.png", f.read(), "image/png")}
    )
test("Status code is 200", r.status_code == 200, f"got {r.status_code}")
grade_data = r.json()
...
# Filter by exam_id
r2 = client.get(f"/api/submissions?exam_id={exam_id}")
...
r_subs = client.get(f"/api/submissions?exam_id={exam_id}")
