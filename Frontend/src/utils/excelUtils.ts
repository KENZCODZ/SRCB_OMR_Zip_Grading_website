...
export function exportSingleSubmissionExcel(
  ...
  const trans = calculateTransmutedGrade(submission.score, submission.total_questions || 50);
  ...
  // Sheet 1: Individual Submission Details Header Card & Summary
  const summaryRows = [
    { 'METRIC / FIELD': 'Report Type', 'VALUE': 'Individual Student Scanned Submission' },
    { 'METRIC / FIELD': 'Exam Title', 'VALUE': examName },
    ...
    { 'METRIC / FIELD': 'Course & Section', 'VALUE': matched?.course_section || 'N/A' },
    { 'METRIC / FIELD': 'Student Email', 'VALUE': matched?.email || 'N/A' },
    { 'METRIC / FIELD': 'Raw Score', 'VALUE': `${submission.score} / ${submission.total_questions || 50}` },
  ...
  const qCount = submission.total_questions || 50;
  const qBreakdown = [];
  ...
  wsBreakdown['!cols'] = [{ wch: 14 }, { wch: 22 }, { wch: 24 }, { wch: 20 }, { wch: 16 }];
...
export function exportExamBatchExcel(
  ...
  const avgScore = totalCandidates > 0
    ...
    : 0;
  const passCount = examSubs.filter(s => calculateTransmutedGrade(s.score, s.total_questions || 50).status === 'Passed').length;
  const passRate = totalCandidates > 0 ? `${Math.round((passCount / totalCandidates) * 100)}%` : '0%';

  // Sheet 1: Exam Overview Header
  const overviewRows = [
    { 'METRIC / FIELD': 'Examination Title', 'VALUE': exam.name },
    { 'METRIC / FIELD': 'Examination Type', 'VALUE': examType },
    { 'METRIC / FIELD': 'Total Students Scanned', 'VALUE': totalCandidates },
    { 'METRIC / FIELD': 'Mean Class Score', 'VALUE': `${avgScore} / 50` },
  ...
  wsSummary['!cols'] = [
    { wch: 16 }, { wch: 25 }, { wch: 18 }, { wch: 32 }, { wch: 20 },
    { wch: 12 }, { wch: 12 }, { wch: 18 }, { wch: 18 }, { wch: 12 }, { wch: 22 }, { wch: 22 }
  ];

  // Sheet 2: Student Item Matrix (Q1 to Q50)
  ...
  // Sheet 3: OBE Item Analysis Report
...
/**
 * 3. Complete Database Export: Exports every scanned submission across all examinations into one comprehensive Excel file.
 */
export function exportCompleteDatabaseExcel(
  ...
  // Sheet 1: Master All Submissions across All Examinations
  const masterData = submissions.map(sub => {
    ...
    return {
      ...
      'Course & Section': matchedStudent?.course_section || 'N/A',
      'Raw Score': sub.score,
      'Total Items': sub.total_questions || 50,
      'Equiv Percentage': `${trans.percentage}%`,
      'Transmuted Grade': trans.grade,
  ...
  wsMaster['!cols'] = [
    { wch: 14 }, { wch: 32 }, { wch: 20 }, { wch: 16 }, { wch: 25 },
    { wch: 18 }, { wch: 12 }, { wch: 12 }, { wch: 18 }, { wch: 18 },
    { wch: 12 }, { wch: 22 }, { wch: 22 }
  ];

  // Sheet 2: Database Summary per Examination
  ...
  wsExamSummary['!cols'] = [
    { wch: 14 }, { wch: 32 }, { wch: 20 }, { wch: 24 }, { wch: 14 },
  ...
  const studentSummaries = roster.map(student => {
    const studentSubs = submissions.filter(s => (s.student_id || '').toLowerCase() === student.student_id.toLowerCase());
  ...
  const workbook = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(workbook, wsMaster, 'All Database Submissions');
  ...
  const fileName = `Complete_Teacher_Database_Report_${dateStr}.xlsx`;
  XLSX.writeFile(workbook, fileName);
}
...
export function exportCHEDGradeSheet(
  ...
  worksheet['!cols'] = [
    { wch: 16 }, { wch: 25 }, { wch: 18 }, { wch: 12 }, { wch: 12 },
